...
# 二行目にアイテム名
    $data modify block ~ ~ ~ front_text.messages[1] set value '{"text":"購入($(Count)個)","clickEvent":{"action":"run_command","value":"/function sss:click/buy/ {Count:$(Count),value:$(value)}"}}'
# 三行目に価格    
